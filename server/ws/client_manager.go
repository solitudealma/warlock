...
func (manager *ClientManager) AddToRooms(roomName string, client *Client) {
	...
	//	fmt.Printf("AddToRooms: key %s\n", k)
	//	for _, roomInfo := range v {
	//		fmt.Printf("AddToRooms: value %+v\n", roomInfo)
	//	}
	//}
}
...
func (manager *ClientManager) EventLogin(Login *Login) {
	...
	//AllSendMessages(Login.AppId, Login.UserId, ws.GetTextMsgDataEnter(Login.UserId, Login.UserId+"-login", "哈喽~"))
}
