...
func NewJWT() *JWT {
	return &JWT{
		[]byte(global.WlConfig.JWT.SigningKey),
	}
}
...
func (j *JWT) CreateClaims(baseClaims request.BaseClaims) request.CustomClaims {
	claims := request.CustomClaims{
		...
		BufferTime: global.WlConfig.JWT.BufferTime,
		RegisteredClaims: jwt.RegisteredClaims{
			// 签名生效时间
			NotBefore: jwt.NewNumericDate(time.Now().Add(-1000 * time.Second)),
			// 过期时间 7天  配置文件
			ExpiresAt: jwt.NewNumericDate(time.Now().Add(time.Duration(global.WlConfig.JWT.ExpiresTime) * time.Second)),
			// 签名的发行者
			Issuer: global.WlConfig.JWT.Issuer,
		},
	}
	return claims
}
